package sudark2.Sudark.craftGod.Menus.createMenus;

import org.bukkit.Bukkit;
import org.bukkit.Material;
import org.bukkit.block.data.BlockData;
import org.bukkit.entity.BlockDisplay;
import org.bukkit.entity.Player;
import org.bukkit.inventory.ItemStack;
import sudark2.Sudark.craftGod.Mark.Mark;

import java.util.ArrayList;
import java.util.List;

import static sudark2.Sudark.craftGod.BlockMenu.*;
import static sudark2.Sudark.craftGod.CraftGod.get;
import static sudark2.Sudark.craftGod.Listeners.BuildingCreate.BuildingTemplate;
import static sudark2.Sudark.craftGod.Menus.createMenus.menuController.checkItem;
import static sudark2.Sudark.craftGod.Menus.createMenus.menuController.displayTemplate;

public class circleSpawner {

    private final static List<ItemStack> circleOptions = List.of(
            nameItem(Material.QUARTZ_BLOCK, "高度+"),
            nameItem(Material.QUARTZ_SLAB, "高度-"),
            nameItem(Material.GOLD_BLOCK, "确认"),
            nameItem(Material.SMOOTH_QUARTZ, "宽度+"),
            nameItem(Material.QUARTZ_SLAB, "宽度-"),
            nameItem(Material.QUARTZ_SLAB, "长度-"),
            nameItem(Material.SMOOTH_QUARTZ, "长度+")
    );

    public static void menu(Player p) {
        create(p, -1, -1, -1, null);
    }

    public static void create(Player pl, int height, int length, int width, Material material) {
        if (height == -1 || length == -1 || width == -1) {
            height = 2;
            length = 2;
            width = 2;
        }
        List<Mark> marks = createSphere(height, length, width,checkItem(material, Material.SMOOTH_QUARTZ));
        List<BlockDisplay> preview = displayTemplate(pl, marks);

        int finalHeight = height;
        int finalLength = length;
        int finalWidth = width;
        menuInit(pl, spawnMenu(circleOptions, pl))
                .thenAccept(
                        index -> {
                            Bukkit.getScheduler().runTask(get(), () -> preview.forEach(BlockDisplay::remove));
                            switch (index) {
                                case -2 ->
                                        create(pl, finalHeight, finalLength, finalWidth, pl.getItemInHand() == null ? null : pl.getItemInHand().getType());
                                case 0 -> create(pl, finalHeight + 1, finalLength, finalWidth, null);
                                case 1 -> create(pl, finalHeight - 1, finalLength, finalWidth, null);
                                case 2 -> {
                                    BuildingTemplate.put(pl.getName(), marks);
                                    menuLoc.remove(pl.getName());
                                }
                                case 3 -> create(pl, finalHeight, finalLength + 1, finalWidth, null);
                                case 4 -> create(pl, finalHeight, finalLength - 1, finalWidth, null);
                                case 5 -> create(pl, finalHeight, finalLength, finalWidth - 1, null);
                                case 6 -> create(pl, finalHeight, finalLength, finalWidth + 1, null);
                            }
                        }
                );
    }

    public static List<Mark> createSphere(int height, int length, int width, BlockData data) {
        List<Mark> marks = new ArrayList<>();

        int hx = length / 2;
        int hz = width / 2;

        float rx = (length + 1) / 2.0f;  // 半径X
        float ry = (height + 1) / 2.0f;  // 半径Y
        float rz = (width + 1) / 2.0f;  // 半径Z

        for (int y = 0; y < height; y++) {
            for (int x = -hx; x <= hx; x++) {
                for (int z = -hz; z <= hz; z++) {
                    float dx = (x + 0.5f) / rx;
                    float dy = (y + 0.5f) / ry;
                    float dz = (z + 0.5f) / rz;

                    if (dx * dx + dy * dy + dz * dz <= 1.0f) {
                        marks.add(new Mark(x, y, z, data));
                    }
                }
            }
        }
        return marks;
    }
}
